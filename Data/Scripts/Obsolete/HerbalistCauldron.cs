			...
			for ( int x = -range; (!burning) && x <= range; ++x )
			{
				for ( int y = -range; (!burning) && y <= range; ++y )
				{
					...
					for ( int i = 0; (!burning) && i < tiles.Length; ++i )
					{
						...
						bool isFlame = ( ( id >= 0x5771 && id <= 0x5775 ) || ( id >= 0x576A && id <= 0x576E ) || ( id >= 0xDE3 && id <= 0xDE9 ) || ( id >= 0x461 && id <= 0x48E ) || ( id >= 0x92B && id <= 0x96C ) || ( id == 0xFAC ) || ( id >= 0x184A && id <= 0x184C ) || ( id >= 0x184E && id <= 0x1850 ) || ( id >= 0x398C && id <= 0x399F ) );
		...
		private class IngredientTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( m_Stage == 4 )
				{
					...
					if ( obj is WaterBottle || obj is Waterskin || obj is DirtyWaterskin )
					{
				...
				else if ( m_Stage == 5 && obj is MixingSpoon )
				{
					...
					{
						int nFilled = (int)( (from.Skills[SkillName.Alchemy].Value + from.Skills[SkillName.Cooking].Value + from.Skills[SkillName.Druidism].Value)/30 );
						if ( nFilled < 1 ){ nFilled = 1; }
						int nHigh = (int)( from.Skills[SkillName.Alchemy].Value + from.Skills[SkillName.Cooking].Value );
						if ( from.CheckSkill( SkillName.Druidism, 0, 100 ) ) { nHigh = nHigh + (int)from.Skills[SkillName.Druidism].Value; }
						nHigh = nHigh / 7;
