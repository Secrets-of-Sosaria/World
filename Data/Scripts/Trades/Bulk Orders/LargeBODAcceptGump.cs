...
namespace Server.Engines.BulkOrders
{
	public class LargeBODAcceptGump : Gump
	{
		...
		public LargeBODAcceptGump( Mobile from, LargeBOD deed ) : base( 50, 50 )
		{
			...
			AddBackground( 25, 10, 430, 240 + (entries.Length * 24), 0x1453 );
				...
				if ( m_From.PlaceInBackpack( m_Deed ) )
				{
					...
					for (int i = 0; i < m_Deed.Entries.Length; ++i )
					{
						if ( m_Deed is LargeSmithBOD )
							m_From.PlaceInBackpack ( new SmallSmithBOD( null, m_Deed.Material, m_Deed.AmountMax, m_Deed.RequireExceptional, m_Deed.Entries[i] ) );
						else
							m_From.PlaceInBackpack ( new SmallTailorBOD( null, m_Deed.Material, m_Deed.AmountMax, m_Deed.RequireExceptional, m_Deed.Entries[i] ) );
					}
				}
				else
				{
